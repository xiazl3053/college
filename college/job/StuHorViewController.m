...
@interface StuHorViewController()<UITableViewDataSource,UITableViewDelegate>
...
@implementation StuHorViewController
...
-(void)updateEducation
{
    ...
    {
        NSDictionary *dict = @{@"time":stuhor.strTime,@"name":stuhor.strName,@"dengji":(stuhor.strLevel==nil)?@"":stuhor.strLevel};
        [array addObject:dict];
    }
    [dict setValue:array forKey:@"_zhengshu"];
...
-(void)addEducation
{
    AddStuHorViewController *addView = [[AddStuHorViewController alloc] initWithAdd:_model];
    
    [self presentViewController:addView animated:YES completion:nil];
}
-(void)updateLoad
{
    __weak StuHorViewController *__self = self;
...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateLoad) name:MESSAGE_FOR_STUHOR_VC object:nil];
    
    [self setTitleText:@"学生奖励"];
    ...
    [btnAdd setTitle:@"+ 添加学生奖励" forState:UIControlStateNormal];
    ...
    if (_model.aryStuHor==nil)
    {
        _model.aryStuHor = [NSMutableArray array];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    XStuHor *edu = [_model.aryStuHor objectAtIndex:indexPath.row];
    
    [cell.lblSchool setText:edu.strName];
...
{
    XStuHor *eduModel = [_model.aryStuHor objectAtIndex:indexPath.row];
    AddStuHorViewController *addView = [[AddStuHorViewController alloc] initWithModel:eduModel];
    [self presentViewController:addView animated:YES completion:nil];
}

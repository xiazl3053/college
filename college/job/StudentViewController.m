...
@interface StudentViewController ()<UITableViewDataSource,UITableViewDelegate>
...
-(void)updateEducation
{
    ...
    for (XEducation *edu in _model.aryEduca)
    {
        
        NSDictionary *dict = @{@"time":[NSString stringWithFormat:@"%@-%@",edu.strStart,edu.strEnd],
                               @"school":edu.strSchool,@"xueli":edu.strLevel,@"zhuanye":@"test"};
        [array addObject:dict];
    }
    [dict setValue:array forKey:@"_education"];
...
-(void)addEducation
{
    AddEvalutionViewController *addView = [[AddEvalutionViewController alloc] initWithAdd:_model];
    
    [self presentViewController:addView animated:YES completion:nil];
}
-(void)updateLoad
{
    __weak StudentViewController *__self = self;
...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateLoad) name:MESSAGE_FOR_EVALUTION_VC object:nil];
    
    [self setTitleText:@"教育经历"];
    ...
    [btnAdd setTitle:@"+ 添加教育经历" forState:UIControlStateNormal];
    ...
    if (_model.aryEduca==nil)
    {
        _model.aryEduca = [NSMutableArray array];
...
{
    return _model.aryEduca.count;
}

-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    static NSString *strStudentViewIdentifier = @"kSTUDENTVIEWIDENTIFIER";
    ...
    XEducation *edu = [_model.aryEduca objectAtIndex:indexPath.row];
   
    [cell.lblSchool setText:edu.strSchool];
    
    [cell.lblTime setText:[NSString stringWithFormat:@"%@-%@",edu.strStart,edu.strEnd]];
...
{
    XEducation *eduModel = [_model.aryEduca objectAtIndex:indexPath.row];
    
    AddEvalutionViewController *addView = [[AddEvalutionViewController alloc] initWithModel:eduModel];
